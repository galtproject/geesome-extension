...
this.singlefile.extension.ui.content.main =
  this.singlefile.extension.ui.content.main ||
  (() => {
      ...
      onStartPage(options) {
        ...
        if (!maskElement) {
          ...
          if (options.shadowEnabled) {
            ...
            maskElement.style.setProperty('opacity', 0.3, 'important');
    ...
    function unmarkSelection() {
      document
        .querySelectorAll('[' + SingleFile.SELECTED_CONTENT_ATTRIBUTE_NAME + ']')
        .forEach(selectedContent => selectedContent.removeAttribute(SingleFile.SELECTED_CONTENT_ATTRIBUTE_NAME));
    }
    ...
    function createProgressBarElement(maskElement) {
      ...
      if (!progressBarElementContainer) {
        ...
        progressBarElement.style.setProperty(
          'background',
          'white linear-gradient(-45deg, rgba(0, 0, 0, 0.1) 25%, transparent 25%, transparent 50%, rgba(0, 0, 0, 0.1) 50%, rgba(0, 0, 0, 0.1) 75%, transparent 75%, transparent) repeat scroll 0% 0% / 50px 50px padding-box border-box',
          'important'
        );
