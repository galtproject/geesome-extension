...
(() => {
  ...
  addEventListener(LOAD_DEFERRED_IMAGES_START_EVENT, () => {
    ...
    const scrollHeight = Math.max(document.documentElement.scrollHeight - clientHeight * 0.5, clientHeight);
    const scrollWidth = Math.max(document.documentElement.scrollWidth - clientWidth * 0.5, clientWidth);
  ...
  addEventListener(LOAD_DEFERRED_IMAGES_START_EVENT, () => {
    ...
    Array.from(observers, ([intersectionObserver, observer]) => {
      ...
      observer.callback(
        observedElements.get(intersectionObserver).map(target => {
  ...
  window.cancelAnimationFrame.toString = function() {
    return 'cancelAnimationFrame() { [native code] }';
  };
  ...
  if (window.IntersectionObserver) {
    ...
    if (observeIntersection && unobserveIntersection) {
      ...
      window.IntersectionObserver.toString = function() {
