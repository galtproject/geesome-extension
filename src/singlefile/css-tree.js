...
this.singlefile.lib.vendor.cssTree =
  this.singlefile.lib.vendor.cssTree ||
  (() => {
    ...
    function firstCharOffset(source) {
      ...
      if (
        source.charCodeAt(0) === 0xfeff || // UTF-16BE
    ...
    Tokenizer.prototype = {
      ...
      error: function(message, offset) {
        const location =
          typeof offset !== 'undefined' && offset < this.source.length
    ...
    const Value_Element = function() {
      ...
      const children = this.createSingleNodeList(this.IdSelector());
    ...
    function isDeclarationBlockAtrule() {
      for (let offset = 1, type; (type = this.scanner.lookupType(offset)); offset++) {
    ...
    function isCustomProperty(str, offset) {
      ...
      return str.length - offset >= 2 && str.charCodeAt(offset) === NAMES_HYPHENMINUS && str.charCodeAt(offset + 1) === NAMES_HYPHENMINUS;
    }
    ...
    function getVendorPrefix(str, offset) {
      ...
      if (str.length - offset >= 3) {
        ...
        if (str.charCodeAt(offset) === NAMES_HYPHENMINUS && str.charCodeAt(offset + 1) !== NAMES_HYPHENMINUS) {
