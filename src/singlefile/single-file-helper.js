...
this.singlefile.lib.helper =
  ...
  (() => {
    ...
    function getElementsInfo(
      ...
      element,
      options,
      data = { usedFonts: new Map(), canvases: [], images: [], posters: [], shadowRoots: [], imports: [], markedElements: [] },
      ascendantHidden
    ) {
      ...
      elements.forEach(element => {
        ...
        if (element.shadowRoot) {
          ...
          if (element.shadowRoot.adoptedStyleSheets && element.shadowRoot.adoptedStyleSheets.length) {
            shadowRootInfo.adoptedStyleSheets = Array.from(element.shadowRoot.adoptedStyleSheets, stylesheet =>
              Array.from(stylesheet.cssRules, cssRule => cssRule.cssText).join('\n')
            );
          }
        }
