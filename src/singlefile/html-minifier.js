...
this.singlefile.lib.modules.htmlMinifier =
  this.singlefile.lib.modules.htmlMinifier ||
  (() => {
    ...
    const modules = [
      ...
      node =>
        mergeElements(
          node,
          'style',
          (node, previousSibling) => node.parentElement && node.parentElement.tagName == 'HEAD' && node.media == previousSibling.media && node.title == previousSibling.title
        ),
    ];
